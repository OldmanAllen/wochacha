...
public final class CameraManager {
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			/*
			 * if (width < MIN_FRAME_WIDTH) { width = MIN_FRAME_WIDTH; } else if (width > MAX_FRAME_WIDTH) { width =
			 * MAX_FRAME_WIDTH; }
			 */
			int height = screenResolution.y * 7 / 10;
			/*
			 * if (height < MIN_FRAME_HEIGHT) { height = MIN_FRAME_HEIGHT; } else if (height > MAX_FRAME_HEIGHT) {
			...
			int topOffset = (screenResolution.y - offsetTop - height) / 2;
	...
	public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
		...
		case PixelFormat.YCbCr_422_SP:
			...
			// rect.height());
			return new PlanarYUVLuminanceSource(data, width, height, rect.left, rect.top, rect.right, rect.bottom);
		default:
	...
	public void setThumbnail(Bitmap barcode) {
		if (bitmap != null) {
			bitmap.recycle();
			bitmap = null;
		}
